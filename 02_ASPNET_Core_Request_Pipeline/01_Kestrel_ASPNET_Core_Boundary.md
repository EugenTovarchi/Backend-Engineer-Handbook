...
приходит HTTP request
  ↓
на границе инфраструктуры размещения и запуска ASP.NET Core создаётся HttpContext
  ↓
...
означает, что web server принимает входящие подключения на указанном адресе, порту и протоколе.
