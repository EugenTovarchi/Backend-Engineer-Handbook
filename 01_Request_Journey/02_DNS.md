...
Если IP по этому имени не найден, Nginx не сможет отправить запрос дальше. Результатом часто будет ошибка proxy, например:
...
Например, внутри Docker контейнеры часто обращаются друг к другу по имени сервиса. Если для имени сервиса не найден IP-адрес, приложение или reverse proxy не сможет подключиться к нужному сервису.
