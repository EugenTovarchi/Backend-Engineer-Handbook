...
**ClaimsIdentity (identity — установленная информация о личности пользователя с набором claims и признаком аутентификации)** описывает одну identity.
...
Если credentials отсутствуют, это само по себе не всегда немедленно завершает request. User может остаться неаутентифицированным, а решение о защищённом endpoint обычно принимает authorization.
...
Authentication в ASP.NET Core pipeline устанавливает, кто отправил request. `UseAuthentication()` пытается выполнить default authenticate scheme, например Bearer или Cookies, и при успехе записывает `ClaimsPrincipal` в `HttpContext.User`. Отсутствие credentials не всегда сразу завершает request. Если выбранный endpoint требует аутентифицированного пользователя, authorization инициирует challenge, а authentication handler формирует ответ конкретной схемы; для Bearer API это обычно `401`. Важно не смешивать это с authorization: authentication отвечает за установление пользователя, а authorization проверяет доступ. JWT, refresh token, OAuth/OIDC и AuthService — отдельная тема Модуля III.
