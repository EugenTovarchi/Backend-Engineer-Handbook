...
**Прогресс до главы:** 78% (7 из 9 глав завершены)
...
- [Refresh Token](../03_ASPNET_Core/14_Authentication.md)
...
**Forwarded headers (проброшенные заголовки — заголовки, через которые прокси передаёт исходные параметры запроса)** помогают backend-приложению понять исходные параметры запроса до обратного прокси.
...
В таком сценарии Nginx или другой входной обратный прокси может выполнять TLS termination:
...
### Ошибка 3. Не настраивать forwarded headers за обратным прокси
...
`X-Forwarded-Proto` — это header, который обратный прокси может передать backend-приложению, чтобы указать исходную схему запроса. Например, клиент пришёл на Nginx по HTTPS, а Nginx отправил запрос в Kestrel по HTTP. Тогда `X-Forwarded-Proto: https` помогает backend понять, что внешний запрос был защищённым.
...
**Модуль I:** `Путешествие одного запроса`  
**Прогресс после главы:** 89% (8 из 9 глав завершены).
