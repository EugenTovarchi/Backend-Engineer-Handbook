...
- часть metadata, которую используют правила сопоставления (matcher policies), например HTTP method metadata.
...
`UseRouting()` выполняет endpoint matching. В современной minimal-модели запуска routing middleware часто добавляется framework автоматически. Явный `UseRouting()` нужен, когда разработчику важно контролировать расположение middleware между matching и endpoint execution.
