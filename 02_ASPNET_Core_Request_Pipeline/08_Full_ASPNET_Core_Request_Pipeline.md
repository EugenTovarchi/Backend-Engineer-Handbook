...
sequenceDiagram
    ...
    Kestrel->>Context: ASP.NET Core Hosting / HttpContext creation
...
app.Use(async (context, next) =>
{
    ...
    finally
    {
        ...
        Console.WriteLine(
            $"{context.Request.Path} -> {context.Response.StatusCode} in {elapsed.TotalMilliseconds} ms");
    }
});
...
Пример показывает порядок, но не означает, что каждый request обязан пройти одинаковый путь. Явный `UseRouting()` показывает место endpoint matching. В современном `WebApplication` routing и endpoint middleware могут добавляться framework автоматически; `MapGet` регистрирует endpoint, а framework добавляет выполнение endpoint в pipeline. Например, `/health` может разрешать anonymous access, а exception middleware может обработать ошибку раньше ответа клиенту.
...
Middleware A calls next
  Middleware B writes response without next
...
| Endpoint разрешает anonymous access | authorization не требует authenticated user |
| Custom middleware вернул ошибку | `400 Bad Request` |
| Routing не нашёл endpoint | `404 Not Found` |
| Path есть, method не поддерживается | `405 Method Not Allowed` |
| Protected endpoint без authenticated user | authorization triggers challenge, Bearer handler usually returns `401` |
| Authenticated user без нужных прав | authorization triggers forbid, Bearer handler usually returns `403` |
...
|---|---|---|
| `404` | Routing | path не сопоставился |
| `405` | Routing / HTTP method matching | path существует, method не поддерживается |
| `401` | Authorization challenge / Authentication scheme | защищённый endpoint, нет валидной identity |
| `403` | Authorization forbid | identity есть, требования доступа не выполнены |
...
Endpoint может не выполниться, если middleware завершил request досрочно, routing не нашёл endpoint, method matching вернул `405`, authorization инициировал challenge/forbid или произошла ошибка раньше endpoint execution.
...
Нужно идти по слоям: сначала понять, дошёл ли request до ASP.NET Core, затем проверить middleware, routing, authentication, authorization и endpoint execution. `404` чаще связан с path matching, `405` — с HTTP method matching, `401` — с authorization challenge и authentication scheme, `403` — с authorization forbid, `500` — с exception в middleware или прикладной логике. Такой подход отделяет infrastructure issues от application pipeline issues.
...
ASP.NET Core request pipeline — это цепочка обработки `HttpContext` после того, как request попал в приложение через Kestrel и ASP.NET Core hosting создал context запроса. Обычно request проходит через middleware, routing выбирает endpoint, authentication пытается установить `HttpContext.User`, authorization проверяет доступ по endpoint metadata, а затем выполняется endpoint: controller action, Minimal API handler, health check или другой обработчик. Если protected endpoint требует пользователя, но валидной identity нет, authorization инициирует challenge, а Bearer handler обычно возвращает `401`. Важно помнить, что это не жёсткая обязательная цепочка для любого запроса: middleware может завершить request досрочно, pipeline может ветвиться, endpoint может разрешать anonymous access, а controller не является обязательным элементом. Response возвращается наружу через middleware, которые вызвали `next`.
