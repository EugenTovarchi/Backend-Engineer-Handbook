...
после next -> response возвращается наружу
```
...
**Short-circuiting (досрочное завершение — ситуация, когда middleware формирует response и не вызывает следующий компонент)** используется для ошибок, redirects, статических файлов, endpoint-ов проверки состояния приложения и других сценариев.
