...
ASP.NET Core Hosting — инфраструктура размещения и запуска ASP.NET Core, которая находится между веб-сервером и application pipeline.
...
| `401` | Authorization challenge / Authentication scheme | защищённый endpoint, identity пользователя не установлена |
...
ASP.NET Core request pipeline — это цепочка обработки `HttpContext` после того, как request попал в приложение через Kestrel и инфраструктура размещения и запуска ASP.NET Core создала context запроса. Обычно request проходит через middleware, routing выбирает endpoint, authentication пытается установить `HttpContext.User`, authorization проверяет доступ по endpoint metadata, а затем выполняется endpoint: controller action, Minimal API handler, health check или другой обработчик. Если защищённый endpoint требует пользователя, но identity не установлена, authorization инициирует challenge, а Bearer handler обычно возвращает `401`. Важно помнить, что это не жёсткая обязательная цепочка для любого запроса: middleware может завершить request досрочно, pipeline может ветвиться, endpoint может разрешать анонимный доступ, а controller не является обязательным элементом. Response возвращается наружу через middleware, которые вызвали `next`.
