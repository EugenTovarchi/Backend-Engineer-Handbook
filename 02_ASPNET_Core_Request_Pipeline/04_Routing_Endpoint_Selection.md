...
**Текущий вопрос:**  
Как ASP.NET Core определяет обработчик для HTTP-метода и path?
...
> Какой endpoint соответствует этому HTTP-методу и path?
...
**Routing (маршрутизация — выбор endpoint по path, HTTP-методу и route template)** определяет, какой обработчик может выполнить request.
...
В обоих случаях ASP.NET Core создаёт набор endpoint-ов приложения (endpoint collection).
...
- HTTP-метод;
- route constraints;
- приоритет маршрутов (route precedence);
- часть metadata, которую используют правила сопоставления (matcher policies), например metadata HTTP-метода.

Упрощённо:

- Сегменты с фиксированным текстом (literal segment) обычно считаются более конкретными, чем сегменты-параметры (parameter segment).

```text
/api/files/list — фиксированный сегмент `list`;
/api/files/{id} — сегмент-параметр `{id}`.
```

- constraints помогают различать кандидатов;
- явный порядок маршрутов (route order) применяется в отдельных сценариях;
...
`UseRouting()` выполняет сопоставление endpoint. В современной minimal-модели запуска routing middleware часто добавляется инфраструктурой ASP.NET Core автоматически. Явный `UseRouting()` нужен, когда разработчику важно контролировать расположение middleware между сопоставлением и выполнением endpoint.
...
Не вся metadata участвует в выборе endpoint. Например, metadata HTTP-метода может участвовать через matcher policies. А `[Authorize]`, policies и `AllowAnonymous` обычно читаются позже authorization middleware: authorization metadata не выбирает endpoint, а описывает требования к уже выбранному endpoint.
...
- внешний прокси-сервер не переписал prefix;
- endpoint не зарегистрирован;
- route constraint не пропустил значение.

`405 Method Not Allowed` обычно означает, что path существует, но endpoints для него не поддерживают HTTP-метод. В таком ответе может быть заголовок `Allow` со списком поддерживаемых методов.
...
Если несколько endpoint-ов одинаково подходят под request, может возникнуть неоднозначное совпадение (ambiguous match), обычно как `AmbiguousMatchException`.
...
flowchart TD
    ...
    Match -- Путь есть, HTTP-метод нет --> MethodNotAllowed[405 + Allow header]
...
Как правильно: разделять выбор endpoint и endpoint execution.
...
Ошибка: не учитывать HTTP-метод.
Почему неверно: `GET /api/files/123` и `POST /api/files/123` могут вести себя по-разному, а path без поддерживаемого HTTP-метода обычно ведёт к `405`, не к обычному `404`.
Как правильно: проверять path, HTTP-метод и заголовок `Allow` вместе.
...
Routing выбирает endpoint по HTTP-методу, path и route template. Он определяет, какой обработчик подходит для запроса.

</details>

---

### Middle: Чем значения маршрута отличаются от query string?

<details>
<summary>Ответ</summary>

Значения маршрута извлекаются из route template, например `{id}` в `/api/files/{id}`. Query string — это параметры после `?`, например `?page=2`.
...
Routing в ASP.NET Core выбирает endpoint по path, HTTP-методу и route template. Endpoint может быть controller action, Minimal API handler, health check или другой обработчик. Routing не выполняет endpoint, а только выбирает его и записывает значения маршрута и metadata в `HttpContext`. Часть metadata может участвовать в сопоставлении, но authorization metadata вроде `[Authorize]` обычно читается позже. Если path не сопоставился, типичный результат — `404`; если path существует, но HTTP-метод не поддерживается, типичный результат — `405 Method Not Allowed`.
...
- Значения маршрута появляются после matching.
- Endpoint metadata нужна authorization.
- `404` обычно означает, что path не сопоставился.
- `405` обычно означает, что path существует, но HTTP-метод не поддерживается.
- Заголовок `Allow` может показать поддерживаемые HTTP-методы.
