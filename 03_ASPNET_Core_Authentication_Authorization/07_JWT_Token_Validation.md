...
Не вставляй реальные production tokens в публичные online decoders. Для изучения используй вымышленный token или локальный decoder. JWT может быть bearer credential и раскрывать claims.
...
- почему подписи недостаточно без проверок issuer/audience/lifetime;
...
**JWS (JSON Web Signature — подписанная защита JSON data)** защищает payload подписью или MAC и даёт integrity. Authenticity зависит от того, что validation правильно выбрала trusted issuer, algorithm и key.

**JWE (JSON Web Encryption — зашифрованная защита JSON data)** даёт confidentiality и cryptographic integrity/authentication зашифрованного содержимого.
...
Но не любой JWT всегда состоит из трёх частей: JWE compact serialization имеет другую структуру. В этой главе основной фокус — signed compact JWT как Access Token. Encrypted JWT — отдельная модель и отдельные эксплуатационные компромиссы.
...
Base64url — это encoding, а не encryption. Signed payload обычно читаем. JWS signature даёт integrity, а authenticity появляется только при корректной trust/key validation. Signature не даёт confidentiality. Если claims нельзя показывать client-у или logs, не кладите их в signed JWT как открытые данные.
...
```text
claims можно использовать только после всех необходимых проверок
```
...
`Authority` указывает заранее выбранный trusted issuer/metadata source. Handler использует metadata, чтобы получить configuration и signing keys. URL из token header не становится trusted authority. Production требует HTTPS metadata endpoint и корректную trust configuration.
...
Если нужны дополнительные проверки, используют `TokenValidationParameters`, но не отключают issuer, audience, lifetime или signature validation. Пример ниже — продолжение callback внутри `AddJwtBearer`:
...
|---|---|
| Valid | signature, issuer, audience, lifetime и context-specific checks успешны; principal создан |
...
Ошибка: вставлять production JWT в публичный decoder.
Почему неверно: JWT может быть bearer credential и раскрывать claims.
Как правильно: использовать вымышленные tokens или локальный decoder.
...
Как правильно: проверять issuer, audience, lifetime и context-specific claims.
...
Это ситуация, когда token одного типа принимают в другом контексте, например ID Token используют как Access Token для API. Защита — явные validation profiles, проверки issuer/audience и token typing там, где применимо.
...
JWT — это формат, а не готовая auth-система. Распространённый signed compact JWT состоит из header, payload и signature; payload обычно читаем, потому что Base64url не является encryption. Доверять claims можно только после validation: trusted key/configuration, allowed algorithm, signature, issuer, audience, lifetime и context-specific claims. JWS даёт integrity, а authenticity зависит от корректной trust/key validation; confidentiality для signed JWT нет. В ASP.NET Core `AddJwtBearer` регистрирует bearer handler: он читает token из `Authorization` header, валидирует его и только после этого создаёт `ClaimsPrincipal`. Revocation self-contained JWT не всегда мгновенный, поэтому нужны short lifetime, introspection/reference tokens, denylist, security version или sender-constrained tokens по threat model.
...
- Реальные bearer JWT нельзя вставлять в публичные decoders.
- Base64url не encryption.
- Signature не заменяет проверки issuer/audience/lifetime.
