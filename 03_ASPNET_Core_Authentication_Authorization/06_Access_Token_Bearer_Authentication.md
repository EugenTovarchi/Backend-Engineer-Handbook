...
Стандартный способ предъявления:

```http
Authorization: Bearer <access-token>
```

Header лучше URL, потому что URL часто попадает в browser history, reverse proxy logs, analytics, referer headers и monitoring traces.
...
**Scope (область доступа — строковое разрешение или delegated permission)** ограничивает, что client может запросить.
...
Но scope не заменяет resource authorization. Token со scope `orders.read` не означает автоматический доступ к любому заказу. API всё ещё должен проверить ownership, tenant, role, policy или resource rule.
...
Ошибка: передавать token в URL или логировать его.
Почему неверно: URL и logs часто распространяются шире, чем кажется.
...
Ошибка: resource server сам выпускает token.
Почему неверно: это смешивает роли API и issuer.
Как правильно: issuer выпускает, resource server проверяет.
...
Scope показывает разрешённый тип доступа, например `orders.read`, но не отвечает, можно ли читать конкретный order конкретного tenant-а или user-а. Resource API должен выполнять domain/resource authorization.
...
- Scope не заменяет resource authorization.
- Token не передают в URL.
