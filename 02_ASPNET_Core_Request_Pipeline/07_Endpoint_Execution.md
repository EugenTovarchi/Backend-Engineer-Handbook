...
Статические файлы могут обслуживаться middleware. В современных сценариях static assets также могут быть endpoint-based. В любом случае controller не является обязательным элементом pipeline.
...
В современном `WebApplication` middleware выполнения endpoint часто добавляется framework автоматически. `MapGet`, `MapControllers`, `MapHealthChecks` и похожие методы регистрируют endpoints. Отсутствие явного `UseEndpoints()` в `Program.cs` не означает отсутствие этапа выполнения выбранного endpoint.
